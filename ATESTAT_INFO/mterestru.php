<?php
  session_start();
 ?>

<html>
<head>

  <link rel="stylesheet" href="style.css">
  <link rel="shortcut icon" href="icon.ico">
  <title>Medii-De-Viață | M-TERESTRU</title>
</head>
<body onload="myMove()">
    <ul class="navigation" >
	    <li class="nav-item"><a href="index.php">Acasă</a></li>
	    <li class="nav-item"><a href="maerian.php">Mediul Aerian</a></li>
	    <li class="nav-item"><a href="mterestru.php">Mediul Terestru</a></li>
      <li class="nav-item"><a href="msubpamant.php">Mediul Subteran</a></li>
	  	<li class="nav-item"><a href="macvatic.php">Mediul Acvatic</a></li>
	    <li class="nav-item"><a href="galerie.php">Galerie</a></li>
      <?php
      if(!isset($_SESSION['user'])){
        echo '
      <li class="nav-item"><a href="cont/login.php">Autentificare</a></li>
      <li class="nav-item"><a href="cont/register.php">Creare cont</a></li>
      ';
      }
      ?>
      <li class="nav-item"><a href="cont/test.php">Test</a></li>
    </ul>

  <input type="checkbox" id="nav-trigger" class="nav-trigger" />
  <label for="nav-trigger"></label>

<div class="site-wrap">
  <?php
  if(isset($_SESSION['user'])){
  echo '<div class = "buton"><form action = "cont/logout.php"> '
        .$_SESSION['prenume'].', '.'<input type = "submit" value = "Deconectare">
        </form></div>
        ';
} else if(!isset($_SESSION['nume'])){
  echo '<p style="float: right; font-weight: bold;">Nu sunteți autentificat</p>';
}
   ?>
   <br><br>
   <img src="margine.png" class = "margine-sus" id = "margineSus">
   <div class="bg">
     <a href="mterestru2.php"><img src="next.png" title="Next" class="next"></a>
     <p><u>Mediul Terestru</u></p>

<div class="terestru">
  <div id="reptile"><b><u> Adaptările reptilelor:</b></u><br></div>

<div id="text">
Corpul lor, mai mult sau mai puțin masiv, este înzestrat cu 4 picioare (membre) pentadactile ale căror degete se termină cu gheare. Pielea nu mai are rol în respiraţie, aceasta devine mai groasă, uneori dotată cu plăci cornoase, cu rolul de a proteja corpul de asemenea produce solzi, scuturi si plăci cornoase cu rol de protecție. Au respirație exclusivă pulmonară; au plămânii bine dezvoltați
</div>
<div id="amfib"><u><b>Adaptările amfibienilor:</b></u><br></div>
<div id="text">
La broaștele țestoase și crocodili, derma produce plăci osoase, care dubleza scuturile cornoase. Tegumentul reptilelor este impermeabil si foarte sărac în glande tegumentare. Membrele sunt situate latero-ventral și nu ridică de la sol restul corpului. Ele servesc pentru sprijin pe substratul pe care se târăsc. Ele se înmulțesc prin ouă care rezultă în urma fecundației interne. Ouăle sunt depuse în cuiburi pe locuri uscate și însorite sau îngropate în nisip, sunt protejate de o coajă pergamentoasă sau calcaroasă.
</div>
     </div>
   </div>
   <img src="margine.png" class = "margine-jos" id = "margineJos">

</div>

<script>
function myMove() {

  var elem = document.getElementById("margineSus");
  var elem2 = document.getElementById("margineJos");
  var pos = 110;
  var pos2 = 250;
  var pos3 = 100;
  var pos4 = 255;
  var id = setInterval(frame, 5);
  function frame() {
    if (pos == 50) {
      clearInterval(id);
    } else {
      pos--;
      elem.style.top = pos + "px";

    }

    if(pos2 == 220){
      clearInterval(id);
    } else {
      pos2--;
      elem.style.left = pos2 + "px";
    }

   if(pos3 == 70){
     clearInterval(id);
   } else {
     pos3--;
     elem2.style.bottom = pos3 + "px";
   }

   if(pos4 == 50){
     clearInterval(id);
   } else {
     pos4--;
     elem2.style.right = pos4 + "px";
   }


  }
  elem.style.opacity = 1;
document.getElementById("margineSus").style.transition = "all 2s";
  elem2.style.opacity = 1;
 document.getElementById("margineJos").style.transition = "all 2s";
}
</script>


</body>
</html>
